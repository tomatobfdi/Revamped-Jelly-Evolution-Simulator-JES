    ...
    for level in range(len(ps)):
        for i in range(len(ps[level])):
            ...
            if level == 0:
                ...
                drawarrow = True
            drawSpeciesCircle(gg,s,species_info[s].coor,R,sim,species_info,font,drawarrow,ui,35,35,-18,-10)
...
def displayAllGraphs(screen, sim, ui):
    ...
    if sim.last_gen_run_time >= 0:
        rightText(screen, f"Last gen runtime: {sim.last_gen_run_time:.3f}s", 881,60, WHITE, ui.smallFont)
...
def blitGraphsandMarks(screen, sim, ui):
    ...
    rightText(screen, f"Median: {dist_to_text(median, True, sim.UNITS_PER_METER)}", 1280,60, WHITE, ui.smallFont)
    ...
    for sp in sim.species_pops[a2].keys():
        ...
        if pop[0] >= sim.c_count*sim.S_VISIBLE:
            ...
            speciesY = 350+280*(1-speciesI/sim.c_count)
            name = species_to_name(sp, ui)
            color = speciesToColor(sp, ui)
            OUTLINE = ui.WHITE if sp == top_species else None
            alignText(screen, f"{name}: {pop[0]}", lineX+10, speciesY, color, ui.sacnameFont, 0.0, [ui.BLACK,OUTLINE])
...
def blitGGandMarks(screen, sim, ui):
    ...
    for sp in sim.species_pops[a2].keys():
        ...
        for c in range(circle_count):
            pygame.draw.circle(screen, ui.WHITE, (cx,cy), R+10*(c/1.4), 3)
    ...
    if ui.species_storage is not None:
        ...
        if sp in sim.species_pops[a2]:
            circle_count = 2 if sp == top_species else 1
            for c in range(circle_count):
                pygame.draw.circle(screen, ui.WHITE, ui.storage_coor, 35, 3)
