...
def drawRingLight(w, h, thickness):
    ...
    #pygame.draw.rect(ringlight,BRIGHT,(0,-10,w,thickness))
    ...
    pygame.draw.rect(ringlight,BRIGHT,(0,0,w,h),5)
    return ringlight

def drawpreviewRingLight(w, h, thickness):
    s = math.sin(time.time()*(2*math.pi)*3)*0.5+0.5
    BRIGHT = (255*s,255*s,0,200)
    ringlight = pygame.Surface((w,h), pygame.SRCALPHA, 32)
    #pygame.draw.rect(ringlight,BRIGHT,(0,-10,w,thickness))
    ...
    #pygame.draw.rect(ringlight,BRIGHT,(0,0,thickness,h))
    #pygame.draw.rect(ringlight,BRIGHT,(w-thickness,0,thickness,h))
    pygame.draw.rect(ringlight,BRIGHT,(0,0,w,h),8)
    return ringlight
...
def leftText(theScreen, stri, x, y, color, font):
    alignText(theScreen, stri, x, y, color, font, 0, None)
...
def drawSpeciesCircle(screen, s, coor, R, sim, species_info, font, shouldDrawArrow, ui, w, h, ox, oy): #w, h = width and height of icon rescale
    color = speciesToColor(s, ui)                                                                      #ox, oy = offset x and offset y because Cary couldn't centre the icons!? Come on Cary.
    ...
    tiny_icon = pygame.transform.scale(creature.icons[0], (w,h))
    screen.blit(tiny_icon,(cx+ox,cy+oy))
