...
from jes_shapes import drawRect, drawRingLight, drawpreviewRingLight, drawX, centerText, alignText, rightText, leftText, drawClock, drawSpeciesCircle
...
class UI:
    ...
    _COLUMN_MARGIN, _MOSAIC_DIM, _MENU_TEXT_UP, _CM_MARGIN1, _CM_MARGIN2):
        ...
        self.buttonList = []
        pygame.font.init()
        self.bigFont = pygame.font.SysFont('Arial', 55)
        self.smallFont = pygame.font.SysFont('Arial', 30)
        self.tinyFont = pygame.font.SysFont('Arial', 21)
        self.buttonFont = pygame.font.SysFont('Arial', 17, bold=True)
        self.sacnameFont = pygame.font.SysFont('Arial', 16)
        self.lightboardFont = pygame.font.SysFont('Arial', 10)
        ...
        self.previewLocations = [[419,81.65,180,180],[419,267,180,180],[419,453,180,180]]
    ...
    def addButtonsAndSliders(self):
        ...
        self.showCreaturesButton = Button(self,buttonCoor[0],["Show jellies","Hide jellies"],self.toggleCreatures)
        self.sortButton = Button(self,buttonCoor[1],["Sort by ID","Sort by fitness","Sort by weakness"],self.toggleSort)
        self.styleButton = Button(self,buttonCoor[2],["Jelly Icons", "Species Tiles"],self.toggleStyle)
        ...
        self.ALAPButton = Button(self,buttonCoor[5],["Turn on Autogen","Turn off Autogen"],self.doNothing)
        ...
        for c in range(self.sim.c_count):
            ...
            if creature.iconCoor[1] < self.mosaicScreen.get_height():
                ...
                if not creature.living and self.showXs:
                    color = (255,0,0) if s == 0 else (0,0,0)
    ...
    def drawMovieGrid(self, screen, coor, mask, titles, colors, font):
        ...
        for i in range(LMS):
            ...
            if titles is not None:
                centerText(screen, titles[i], x+W/2, y+H-17, colors[i], font)
    ...
    def drawMovieQuad(self, species):
        ...
        self.drawMovieGrid(self.screen, coor, mask, titles, [self.GRAYISH]*L, self.sacnameFont)
    ...
    def drawInfoBarSpecies(self, species):
        ...
        if species in self.sim.species_pops[now]:
            ...
            extinct_string = " (Alive)"
        strings = [f"Species {s_name}",f"Ancestor {a_name}",f"Lifespan: G{info.getWhen(1)} - G{info.getWhen(3)}{extinct_string}", f"Population: {info.apex_pop} at apex (G{info.getWhen(2)})", f"{now_pop} now (G{now})"] #   |   {now_pop} now (G{now})"]
        placenums = ["17","34","51","68","85","102","119","136","153","170","187","204","221","238","250"]
        placexpos = [332, 345]
        ...
        for i in range(len(strings)):
            ...
            leftText(self.infoBarScreen, strings[i], 10, self.MOVIE_SINGLE_DIM[1]+40+42*i, colors[i], self.smallFont)
        ...
        for i in range(15):
            leftText(self.infoBarScreen, placenums[i], placexpos[i%2], 6.95*(i+1)+361, self.WHITE, self.lightboardFont)

    def drawLightboard(self, screen, species, gen): #, coor):
        DIM = self.MOSAIC_DIM[-1]
        #R = coor[2]/DIM*3
        lightsize = 7
        light_plus_x = 205                              #explanation for the lightboard's cell's x position script
        light_plus_y = 357
        for c in range(self.sim.c_count):               # because for x in range() starts with 0, we + 1 to c
            #x = coor[0]+R*(c%DIM)                      # for the rest of these explanations, c means (c + 1).
            if (c+1)%17 == 0:                           # we get it's x position by taking c%17, because 17 is
                x = (17*lightsize)+light_plus_x         # the length of the creature mosaic. if c was 1 then 
            else:                                       # x position is 1%17 = 1. if c is 2, xpos is 2%17 = 2.
                x = ((c+1)%17*lightsize)+light_plus_x   # This also works for higher numbers, for example 18%17
            #y = coor[1]+R*(c//DIM)                     # = 1, so the eighteenth cell will be the first cell
            if (c+1) <= 17:                             # of the second row.
                y = light_plus_y + lightsize
            elif (c+1) <= 17*2:                         # the reason we have an if (c+1)%17 == 0: is because if
                y = light_plus_y + lightsize*2          # the cell number is a multiple of 17, instead of being
            elif (c+1) <= 17*3:                         # placed at the seventeenth spot of the row, it gets placed
                y = light_plus_y + lightsize*3          # at the zeroth spot of the row. This is because 17%17 = 0,
            elif (c+1) <= 17*4:                         # 34%17 = 0, 170%17 = 0 and etc. so yeah. BUY NOW!!!!
                y = light_plus_y + lightsize*4
            elif (c+1) <= 17*5:
                y = light_plus_y + lightsize*5
            elif (c+1) <= 17*6:
                y = light_plus_y + lightsize*6
            elif (c+1) <= 17*7:
                y = light_plus_y + lightsize*7
            elif (c+1) <= 17*8:
                y = light_plus_y + lightsize*8
            elif (c+1) <= 17*9:
                y = light_plus_y + lightsize*9
            elif (c+1) <= 17*10:
                y = light_plus_y + lightsize*10
            elif (c+1) <= 17*11:
                y = light_plus_y + lightsize*11
            elif (c+1) <= 17*12:
                y = light_plus_y + lightsize*12
            elif (c+1) <= 17*13:
                y = light_plus_y + lightsize*13
            elif (c+1) <= 17*14:
                y = light_plus_y + lightsize*14
            else:
                y = light_plus_y + lightsize*15
            
            col = (0,0,0)
            creature = self.sim.creatures[gen][self.sim.rankings[gen][c]]
            if creature.species == species:
                col = speciesToColor(species, self)
            pygame.draw.rect(screen,col,(x,y,lightsize, lightsize))
        
    def drawMenuText(self):
        y = self.W_H-self.MENU_TEXT_UP + 65
        titleSurface = self.bigFont.render("Jelly Evolution Simulator", False, self.GRAYISH) 
        self.screen.blit(titleSurface,(19,6))
        a = str(int(self.genSlider.val))
        b = str(int(self.genSlider.val_max))
        genSurface = self.smallFont.render("Generation "+a+" / "+b, False, (255,255,255))
        self.screen.blit(genSurface,(40,y))
        if self.species_storage is not None:
            s = self.species_storage
            R = 35 #RADIUS OF SPECIES STORAGE
            drawSpeciesCircle(self.screen,s,self.storage_coor,R,self.sim,self.sim.species_info,self.sacnameFont,False,self,35,35,-18,-10)        #IMPORTANT
    ...
    def drawPreviews(self):
        ...
        if gen >= 0 and gen < len(self.sim.rankings):
            ...
            for r in range(3):
                ...
                DIM = (self.previewLocations[r][3],self.previewLocations[r][3])
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                for slider in self.sliderList:
                    ...
                    if mouseX >= s_x and mouseX < s_x+s_w and mouseY >= (s_y+30) and mouseY < (s_y+30)+(s_h/2):
                        self.sliderDrag = slider
                        break
                for button in self.buttonList:
                    s_x, s_y, s_w, s_h = button.dim
                    if mouseX >= ((s_x/1.5)+430) and mouseX < ((s_x/1.5)+430)+(s_w/1.7) and mouseY >= (s_y+70) and mouseY < (s_y+70)+(s_h/2):
                        button.click()
    ...
    def displayMovies(self, screen):
        ...
        if self.CLH[0] == 1:
            ...
            self.screen.blit(drawpreviewRingLight(DIM[2],DIM[3],6),(DIM[0],DIM[1]))
        else:
    ...
    def drawSlidersAndButtons(self):
        ...
        for button in self.buttonList:
            button.drawButton(self.screen, self.buttonFont)  
    ...
    def toggleCreatures(self, button):
        self.mosaicVisible = (button.setting == 1)                      #IMPORTANT
